...
const dayMap={};
for(const x of ev){const day=(x?.ts||x?.createdAt||'').slice(0,10);if(!day)continue;if(!dayMap[day])dayMap[day]={date:day,visitors:new Set(),qualified:0,clicks:0,communityClicks:0};const z=dayMap[day];if(x?.visitorId)z.visitors.add(x.visitorId);if(x?.event==='qualification_complete')z.qualified++;if(x?.event==='affiliate_click')z.clicks++;if(x?.event==='telegram_join_click'||x?.event==='whatsapp_join_click')z.communityClicks++;}
const daily=Object.values(dayMap).map(x=>({date:x.date,visitors:x.visitors.size,qualified:x.qualified,clicks:x.clicks,communityClicks:x.communityClicks})).sort((a,b)=>b.date.localeCompare(a.date));
const campaignSummary=Object.fromEntries(Object.entries(campaigns).map(([k,v])=>[k,{events:v.events,visitors:v.visitors.size,qualified:v.qualified,clicks:v.clicks}]));return Response.json({visitors,qualified,affiliateClicks,brokerClicks,telegramClicks,whatsappClicks,commission,funnel,dropoffs,diagnostics,verifiedTotals,businessByBroker,campaigns:campaignSummary,acquisition,channels,contents,recommendations,daily,events:ev.length,businessEvents:be.length})};
