...
const campaignSummary=Object.fromEntries(Object.entries(campaigns).map(([k,v])=>[k,{events:v.events,visitors:v.visitors.size,qualified:v.qualified,clicks:v.clicks}]));return Response.json({visitors,qualified,affiliateClicks,brokerClicks,telegramClicks,whatsappClicks,commission,funnel,dropoffs,diagnostics,verifiedTotals,businessByBroker,campaigns:campaignSummary,acquisition,channels,contents,recommendations,events:ev.length,businessEvents:be.length})};
